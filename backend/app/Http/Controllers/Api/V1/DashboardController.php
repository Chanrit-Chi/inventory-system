...
class DashboardController extends BaseApiController
{
    ...
    public function summary(Request $request): JsonResponse
    {
        ...
        // 6. Low Stock Products: Count of active products where total stock <= reorder_level
        $lowStockSkus = (int) Product::query()
            ->whereNull('deleted_at')
            ->where('is_active', true)
            ->whereRaw('(SELECT COALESCE(SUM(pv.quantity_on_hand), 0) FROM product_variants pv WHERE pv.product_id = products.id AND pv.deleted_at IS NULL AND pv.is_active = true) <= COALESCE(products.default_reorder_level, 5)')
            ->count();
